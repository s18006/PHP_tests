...
class createElementClass extends formatElementClass {
    ...
    public function fillIRadioElement($input, $id) {
        ...
        $id_list_label = array();
        foreach ($id as $key) {
            $format_type = explode('=', $key)[0];
            if ($format_type === 'id') {
                $id_value = explode('=', $key)[1];
                $id_list_input[] = $key;
                $id_list_label[] = $key;
            }
            if ($format_type === 'value' || $format_type === 'name' || $format_type='req') {
                $id_list_input[] = $key;
            }
            if ($format_type === 'class') {
                $id_list_input[] = $key;
                $id_list_label[] = $key;
            }
        }
