...
$x = array(
    ...
    array('checkbox', 'M', 'SELECT文に ORDER BY ASC を指定してデータを検索した場合の動作として、 正しいものを2つ選択し てください', '数値は、小さい値が先に表示される', '日付は、新しい日付から先に表示される', '英字は、アルファベット順に表示される', 'NULLは、最初に表示される', hash('sha256', json_encode('数値は、小さい値が先に表示される英字は、アルファベット順に表示される')), ''),
    array('checkbox', 'M', '同時実行制 御の説明で正しいものを 2 つ選んで下さい', '検索対象行を明示的にロックできる', '2人以上のユーザーが互いにロックをかけあうとデットロックが発生する', 'FOR UPDATE句を使用すると複数ユーザーが同じデータに同時アクセスできる', 'ロック待機状態を避ける方法はない', hash('sha256', json_encode('検索対象行を明示的にロックできる2人以上のユーザーが互いにロックをかけあうとデットロックが発生する')), ''),
    array('checkbox', 'M', 'SELECT 処理について、以下の説明文から正しいものを 2 つ選んで下さい', '全列を選択する場合、列名の代わりに「*」を使用できる', '複数の列を問合せた場合、常に表作成時の列の順序に従って検索される', '検索処理において、一時的に列に別名を定義できる', '複数列を問合せる場合、列名と列名の間をスペースで区切る', hash('sha256', json_encode('全列を選択する場合、列名の代わりに「*」を使用できる検索処理において、一時的に列に別名を定義できる')), '')
);
